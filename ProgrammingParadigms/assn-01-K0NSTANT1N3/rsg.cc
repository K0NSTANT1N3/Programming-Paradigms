...
void f1(map <string, Definition> &grammar, string nonterminal, string &sentence) {
    Definition def = grammar[nonterminal];
    Production prod = def.getRandomProduction();
    for (Production::iterator it = prod.begin(); it != prod.end(); ++it) {
        if ((*it)[0] == '<') {
            f1(grammar, *it, sentence);
        } else {
            if (*it == ";") {
//                sentence += ". ";
            }
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < 3; i++) {
        sentences = "";
        f1(grammar, "<start>", sentences);
        cout << "Version #" << i << ": --------------------------" << endl;
        cout << sentences << endl << endl;
    }
